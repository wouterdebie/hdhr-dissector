...
dissect_hdhr(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    ...
    /*** HEURISTICS ***/
    if (tvb_length(tvb) < HDHR_MIN_LENGTH)
        return 0;
...
void
proto_reg_handoff_hdhr(void)
{
    dissector_handle_t hdhr_handle = new_create_dissector_handle(dissect_hdhr, proto_hdhr);
